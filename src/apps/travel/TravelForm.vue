            ...
            <el-form :model='formData' class="demo-form-inline" ref="formupdate">
                ...
                <table style="width: 100%; table-layout: fixed; word-break: break-all;">
                    ...
                    <tr v-for="(item,index) in formData.estimate" :key="index" @contextmenu.prevent="deleteItem(item,index,'cost')">
                        ...
                        <td colspan="1">
                            <el-input @mousewheel.native.prevent type="number" oninput="if(value.length>13) value=value.slice(0,13)" v-model="item.price" @input="getAmount(item)"></el-input>
                        </td>
                        <td colspan="1">
                    ...
                    <tr>
                        ...
                        <td colspan="6" style="padding: 10px;">
                            <div v-for="(item,index) in formData.attachments" :key="item.id" style="float:left">
                                <FilesOperate :item="item" :options="{preview:true,del:true,download:true}" @getId="deleteAttachments(index,formData.attachments,'/api/v1/travel_forms/delAttachment/' + item.id,'get')"></FilesOperate>
                            </div>
                        </td>
                    </tr>
