<template>
    <div id="PartMeetingForm">
        <el-form :model="rows" label-width="100px" :rules="rules" ref="formupdate">
            <el-row>
                <el-col :span="12">
                    <el-form-item label="党委会编号" prop="numbers">
                        <el-input v-model="rows.numbers" placeholder="请输入党委会编号"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                ...
                <el-col :span="24">
                    <el-col :span="24">
                        <el-form-item label="发送" prop="distribute">
                            <el-select style="width:100%;" v-model="rows.distribute" @remove-tag="removeTag" multiple allow-create filterable readonly default-first-option placeholder="请选择分送人员">
                                <el-option v-for="item in users" :key="item.id" :label="item.name" :value="item.name">
                                </el-option>
                            </el-select>
                        </el-form-item>
        ...
        <!-- <el-dialog title="新增议题" :visible.sync="dialogFormDiscussion" width="960px" top="150px" custom-class="discussion" :close-on-click-modal="false" append-to-body>
            <PartDiscussionForm ref="PartDiscussionForm" :discussionData="discussionData" @currentDiscussionData="getDiscussionData"></PartDiscussionForm>
...
import PartDiscussionForm from './PartydiscussionForm';
import FilesOperate from '../FilesOperate';
import PartymeetingFiles from './PartymeetingFiles';
export default {
    name: 'PartMeetingForm',
    data() {
    ...
    methods: {
        ...
        editText() {
            ...
            this.$refs.partymeetingfiles.openData(this.rows.text.url);
        },
        ...
        getForm() {
            ...
            if (self.formId != '') {
                axios
                    .get('/api/v1/party_meeting_forms/' + self.formId)
                    .then(res => {
        ...
        addDiscussion() {
            ...
            this.$nextTick(function() {
                if (this.$refs.PartDiscussionForm) {
                    this.$refs.PartDiscussionForm.clearForm(
        ...
        savaDiscussion(data) {
            this.$refs.PartDiscussionForm.savaCurrentDiscussion(data);
        },
        deleteDiscussion(item, event, index) {
            ...
            this.$confirm('是否删除此?', '提示', {
                ...
                if (item.id) {
                    axios
                        .get(
                            '/api/v1/party_meeting_forms/deleteTask/' + item.id
                        )
                        .then(res => {});
                }
        ...
        comment() {
            ...
            axios
                .put(
                    `/api/v1/party_meeting_forms/${self.currentFormId}/comment`,
                    {
                        content: '提交',
                        action: 'COMMIT',
                        node: '提交'
        ...
        startProcess() {
            ...
            axios
                .put(
                    '/api/v1/party_meetings/' + self.currentFormId + '/signal',
                    params
                )
                .then(res => {
                    self.$emit('refreshData');
        ...
        clearForm() {
            this.rows = {
                ...
                attend: '',
                delegate: '',
                writer: this.$store.getters.LoginData.uname,
                meetingTasks: [],
                attachments: [],
                remarks: '',
                phone: '',
    ...
    mounted() {
        ...
        //  console.log(this.$refs.PartDiscussionForm);
        //  this.$refs.PartDiscussionForm.clearForm(0);
    }
};
</script>
