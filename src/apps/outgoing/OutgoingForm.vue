                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item label="附件">
                                <el-upload name="files" class="upload-demo uploadBtn" ref="upload" action="/api/v1/files/upload" :on-success="handleSuccess" :multiple="true" accept="" :auto-upload="true" :with-credentials="true" :show-file-list="false">
...
export default {
    ...
    methods: {
        getwordNoList() {
            const self = this;
            let onames = {
                oname: decodeURI(this.$store.getters.LoginData.companyName)
            };
            axios
    ...
    mounted() {
        ...
        cookieItems.forEach(function(item) {
            if (item.indexOf('uname') > -1) {
                self.formData.creatorName = decodeURIComponent(
                    item.split('=')[1]
                );
                self.cookie_uname = decodeURIComponent(item.split('=')[1]);
            }
            if (item.indexOf('oname') > -1) {
                self.formData.organName = decodeURIComponent(
                    item.split('=')[1]
                );
