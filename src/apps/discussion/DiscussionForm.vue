                    ...
                    <el-col :span="8">
                        <el-form-item label="会议类型" prop="branchlineTo">
                            <el-select v-model="formData.branchlineTo" placeholder="请选择会议类型">
                                <el-option v-for="item in discussionOption" :key="item.value" :label="item.label" :value="item.value">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
    ...
    methods: {
        ...
        resetForm() {
            let formData = {
                ...
                timeApplication: moment(new Date()).format(
                    'YYYY-MM-DD HH:mm:ss'
                ),
                topicName: '',
        ...
        async saveForm(params) {
            ...
            if (
                !$self.formData.requestedItems[0] ||
                !$self.formData.requestedItems[0].content ||
                $self.formData.requestedItems[0].content.length <= 0
            ) {
