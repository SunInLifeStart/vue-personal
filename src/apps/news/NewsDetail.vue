    ...
    <div id="NewsDetail" :class="{fullScreen:fullScreen}">
        ...
        <div class="formContent">
            ...
            <el-form ref="formupdate" :model="tableData" style="margin-top:10px;">
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="简介：">{{tableData.brief}}</el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="新闻标题：">{{tableData.title}}</el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="副标题：">{{tableData.btitle}}</el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="信息内容：" style="white-space:pre-wrap;">
                            <!--
                            <el-input v-model="tableData.content" type="textarea" disabled :autosize="{minRows: 10}">
                            </el-input>
                            -->
                ...
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="报送单位：">{{tableData.reportingOrg}}</el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="集团门户图片：">
                            <div v-for="item in  tableData.attachments" :key="item.id" style="float:left" v-if="item.attType == 'group'">
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="附件：">
                            <div v-for="item in  tableData.attachments" :key="item.id" style="float:left" v-if="item.attType == 'attachment'">
                                <FilesOperate :item="item" :options="{preview:true,download:true}" @getId="getId"></FilesOperate>
                            </div>
                            <!--
                            <div class="attachments" v-for="item in tableData.attachments" :key="item.id" @click="downloadFile(item.url)">
            ...
            <el-dialog title="新闻管理" :visible.sync="dialogFormVisibleNews" :close-on-click-modal="false" max-width="1280px" width="70%">
                <NewsForm ref="newsform" @saveStatus="saveStatus" :formId="dialogFormId"></NewsForm>
...
export default {
    ...
    methods: {
        ...
        saveStatus(status) {
            this.dialogFormVisibleNews = status;
            this.submitForm();
        },
        downloadFile(item) {
        ...
        getActions() {
            axios.get(`/api/v1/news/${this.formId}/actions`).then(res => {
            ...
            axios.get(`/api/v1/news/${this.formId}/crumb`).then(res => {
        ...
        getRejectList() {
            let self = this;
            axios
                .get('/api/v1/news/' + this.formId + '/reject/targets')
                .then(res => {
        ...
        doAction(action) {
            ...
            if ('ARCHIVE,DISPATCH,TEMPLATE,PULL,COMMIT'.includes(action.type)) {
                ...
                this.dialogFormVisibleNews = true;
            } else {
                this.dialogVisible = true;
            }
        },
        comment(comment) {
            let self = this;
            axios
                .put(`/api/v1/news_forms/${self.formId}/comment`, {
        ...
        submitForm() {
            ...
            if (
                ...
                let toXML = {
                    site: this.tableData.toSite,
                    portal: this.tableData.toPortal,
                    wechat: this.tableData.toWechat,
                    weibo: this.tableData.toWeibo
                };
                for (var i in toXML) {
                    if (toXML[i]) {
                        to.push(i);
                ...
                self.submitData.to = to;
                self.submitData.action = self.currentAction.type;
                console.log(self.submitData.action.type);
                axios
                    .put(`/api/v1/news/${self.formId}/signal`, self.submitData)
                    .then(res => {
            ...
            const params = {
                ...
                source:
                    this.tableData.organName != null
                        ...
                        : null,
                abstract:
                    this.tableData.brief != null ? this.tableData.brief : null,
                tags: '新闻中心',
                url: arrurl,
                img: arrimage,
                about:
        ...
        doComment(action) {
            ...
            axios
                .put(`/api/v1/news_forms/${this.formId}/comment`, {
                    content: comment,
                    action: action.type
                })
                .then(res => {
                    ...
                    if (!'SAVE,PREVIEW,COMMENT'.includes(action.type)) {
                        axios
                            .put(`/api/v1/news/${this.formId}/signal`, {
                                action: action.type,
                                assignees: atIds
                            })
                            .then(res => {
