<template>
    <div id="GmoMeetingDetail" :class="{fullScreen:fullScreen}">
        ...
        <div class="formContent">
                    ...
                    <el-col :span="24">
                        <el-form-item label="议题：">
                            <div class="squ" v-for="(item,index) in tabledata.meetingTasks" :key="item.id" style="margin-left:60px;">
                                <div style="font-weight:bold">{{index | fiterNumber}} {{item.title}}</div>
            ...
            <el-dialog :visible.sync="dialogVisible" center width="30%" append-to-body>
                <el-form :model="formInline" ref="formupdate" :rules="rules">
                    <el-form-item label="请选择驳回节点" v-show="reject_status" prop="rejectTarget">
                        <el-select v-model="formInline.rejectTarget" style="width:100%;">
                    ...
                    <el-form-item label="审批意见">
                           <el-input type="textarea"  placeholder="请输入审批意见" v-model="textarea" :autosize="{ minRows: 10, maxRows: 30}"></el-input>
                    </el-form-item>
                </el-form>
...
export default {
    ...
    methods: {
        ...
        getActions() {
            let self = this;
            axios
                .get(`/api/v1/gmo_meetings/${this.formId}/actions`)
                .then(res => {
        ...
        getCrumbs() {
            axios.get(`/api/v1/gmo_meetings/${this.formId}/crumb`).then(res => {
        ...
        getRejectList() {
            let self = this;
            axios
                .get('/api/v1/gmo_meetings/' + this.formId + '/reject/targets')
                .then(res => {
        ...
        template() {
            ...
            axios({
                method: 'get',
                url: '/api/v1/gmo_meeting_forms/' + this.formId + '/template',
                headers: {
        ...
        doAction(action) {
            ...
            if ('ARCHIVE,DISPATCH,TEMPLATE,PULL,COMMIT'.includes(action.type)) {
                ...
                if (action.type == 'PULL') {
                    axios
                        .get(`/api/v1/gmo_meetings/${self.formId}/pull`)
                        .then(res => {
                            self.comment('formOnlyComment');
                            self.getActions();
                            self.getCrumbs();
        ...
        comment(comment) {
            let self = this;
            axios
                .put(`/api/v1/gmo_meeting_forms/${self.formId}/comment`, {
        ...
        saveForm() {
            if (this.currentAction.type == 'REJECT') {
        ...
        editText(data) {
            if (data == 'createNewDoc') {
                ...
                this.$refs.gmomeetingfiles.openData(this.tabledata.text.url);
            }
        },
        saveFormForText() {
            const self = this;
            ...
            axios
                .post('/api/v1/gmo_meeting_forms/save', self.tabledata)
                .then(res => {
        ...
        submitForm() {
            ...
            if (
                ...
                if (self.currentAction.type == 'REJECT') {
                    if (self.seleteUsers) {
                        self.submitData.rejectTarget =
                            self.formInline.rejectTarget;
                    } else {
                ...
                axios
                    .put(
                        `/api/v1/gmo_meetings/${self.formId}/signal`,
                        self.submitData
                    )
                    .then(res => {
